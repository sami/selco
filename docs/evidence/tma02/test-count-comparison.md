...
npx vitest run --coverage --exclude '**/.claude/**'

# Verbose test names (one line per test)
npx vitest run --reporter=verbose --exclude '**/.claude/**'
```

The `--exclude '**/.claude/**'` flag scopes discovery to project
sources and prevents Vitest from picking up duplicate test copies
inside the local git worktrees under `.claude/worktrees/`. This
