...
- **Ahead:** Phase 1 cleanup (gitignore tightening,
...
under `.worktrees/{kind-driscoll, pedantic-hypatia,
...
6. **TDD as the default after TMA 01** (visible across Era 4). Every new
   ...
   is also encoded as a standing project rule ("TDD mandatory").
...
- `.notes/RULES.md`, `.notes/skills/*`, `.notes/workflows/*` — early agent
...
- **Test suite (`src/`):** 51 test files, 500 test cases (Vitest 4 +
  ...
  also picks up tests inside the local `.worktrees/*` folders;
  see §6 and §10.
...
- **Branches present:** `main`, `feat/decking-integration` (merged into
  main this session, can be deleted), `wt/pedantic-hypatia`
  (previously merged), `wt/kind-driscoll` (older worktree),
  `wt/crazy-rubin` (active audit worktree).
...
1. **A stray editor-rules document at the repo root.** Explicitly
...
11. **Vitest discovery picks up `.worktrees/*` test files.**
